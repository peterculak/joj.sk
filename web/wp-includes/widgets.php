...
function wp_widget_rss_output( $rss, $args = array() ) {
	...
	foreach ( $rss->get_items( 0, $items ) as $item ) {
		...
		if ( $link == '' ) {
			...
			echo "<li><a class='rsswidget' target='_parent' href='$link'>$title</a>{$date}{$summary}{$author}</li>";
		} else {
			echo "<li><a class='rsswidget' target='_parent' href='$link'>$title</a>{$date}{$author}</li>";
		}
	}
	echo '</ul>';
	$rss->__destruct();
