...
function get_category_parents( $id, $link = false, $separator = '/', $nicename = false, $visited = array() ) {
	...
	if ( $link )
		$chain .= '<a target="_parent" href="' . esc_url( get_category_link( $parent->term_id ) ) . '">'.$name.'</a>' . $separator;
	else
		...
		foreach ( $categories as $category ) {
			...
			switch ( strtolower( $parents ) ) {
				case 'multiple':
					...
					$thelist .= '<a target="_parent" href="' . esc_url( get_category_link( $category->term_id ) ) . '" ' . $rel . '>' . $category->name.'</a></li>';
					break;
				case 'single':
					$thelist .= '<a target="_parent" href="' . esc_url( get_category_link( $category->term_id ) ) . '"  ' . $rel . '>';
					if ( $category->parent )
		...
		foreach ( $categories as $category ) {
			...
			switch ( strtolower( $parents ) ) {
				case 'multiple':
					if ( $category->parent )
						$thelist .= get_category_parents( $category->parent, true, $separator );
					$thelist .= '<a target="_parent" href="' . esc_url( get_category_link( $category->term_id ) ) . '" ' . $rel . '>' . $category->name.'</a>';
					break;
				case 'single':
					$thelist .= '<a target="_parent" href="' . esc_url( get_category_link( $category->term_id ) ) . '" ' . $rel . '>';
					if ( $category->parent )
...
function wp_list_categories( $args = '' ) {
	...
	if ( empty( $categories ) ) {
		...
		if ( ! empty( $show_option_all ) ) {
			...
			if ( 'list' == $r['style'] ) {
				$output .= "<li class='cat-item-all'><a target='_parent' href='$posts_page'>$show_option_all</a></li>";
			} else {
				$output .= "<a target='_parent' href='$posts_page'>$show_option_all</a>";
			}
		}
...
function wp_generate_tag_cloud( $tags, $args = '' ) {
	...
	foreach ( $tags_data as $key => $tag_data ) {
		...
		$a[] = "<a target='_parent' href='" . esc_url( $tag_data['url'] ) . "' class='" . esc_attr( $class ) . "' title='" . esc_attr( $tag_data['title'] ) . "' style='font-size: " . esc_attr( str_replace( ',', '.', $tag_data['font_size'] ) . $args['unit'] ) . ";'>" . esc_html( $tag_data['name'] ) . "</a>";
	}
...
function get_the_term_list( $id, $taxonomy, $before = '', $sep = '', $after = '' ) {
	...
	foreach ( $terms as $term ) {
		...
		$links[] = '<a target="_parent" href="' . esc_url( $link ) . '" rel="tag">' . $term->name . '</a>';
	}
