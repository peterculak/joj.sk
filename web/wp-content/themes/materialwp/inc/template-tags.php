...
function materialwp_posted_on() {
	...
	$posted_on = sprintf(
		...
		'<a target="_parent" href="' . esc_url( get_permalink() ) . '" rel="bookmark">' . $time_string . '</a>'
	);

	$byline = sprintf(
		_x( '<i class="mdi-action-account-circle"></i> %s', 'post author', 'materialwp' ),
		'<span class="author vcard"><a class="url fn n" target="_parent" href="' . esc_url( get_author_posts_url( get_the_author_meta( 'ID' ) ) ) . '">' . esc_html( get_the_author() ) . '</a></span>'
	);
